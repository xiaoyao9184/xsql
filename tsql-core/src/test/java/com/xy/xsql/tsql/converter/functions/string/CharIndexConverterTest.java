package com.xy.xsql.tsql.converter.functions.string;

import com.xy.xsql.block.core.printer.MetaBlockPrinter;
import com.xy.xsql.block.model.BlockMeta;
import com.xy.xsql.tsql.builder.chain.functions.string.CharIndexFunctionTest;
import com.xy.xsql.tsql.converter.BaseTest;
import com.xy.xsql.tsql.model.functions.string.CharIndex;
import org.junit.Assert;
import org.junit.Before;
import org.junit.Test;

import java.io.StringWriter;
import java.util.LinkedHashMap;
import java.util.Map;

/**
 * Created by xiaoyao9184 on 2018/5/31.
 */
public class CharIndexConverterTest {

    @Test
    public void testMetaPrint() throws Exception {
        BlockMeta b = CharIndexConverter.meta;

        StringWriter writer = MetaBlockPrinter.print(b);

        System.out.println(writer);
        Assert.assertEquals(writer.toString(),
                "<CHARINDEX> ::=\n" +
                        "CHARINDEX ( expressionToFind , expressionToSearch [ , start_location ] )");
    }

    private Map<CharIndex,String> model2StringMap;

    @Before
    public void init(){
        CharIndexFunctionTest builderTest = new CharIndexFunctionTest();
        model2StringMap = new LinkedHashMap<>();

        model2StringMap.put(
                builderTest.exampleA,
                "CHARINDEX('bicycle', @document)");
        model2StringMap.put(
                builderTest.exampleB,
                "CHARINDEX('vital', @document, 5)");
//        model2StringMap.put(
//                builderTest.exampleD,
//                "CHARINDEX ( 'TEST',\n" +
//                        "     'This is a Test'\n" +
//                        "     COLLATE Latin1_General_CS_AS)");
    }

    @Test
    public void testPrint() throws Exception {
        BaseTest.testPrint(model2StringMap);
    }

    @Test
    public void testKeywordPrint() throws Exception {
        BaseTest.testKeywordPrint(model2StringMap);
    }

}